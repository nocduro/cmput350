...
void Bot::OnUnitIdle(const Unit* unit) {
	...
	switch (unit->unit_type.ToType()) {
		...
		case UNIT_TYPEID::TERRAN_COMMANDCENTER: {
			Actions()->UnitCommand(unit, ABILITY_ID::TRAIN_SCV); // Train scv in command center
			break;
		}

        // if scv is idle, make it do something productive
		case UNIT_TYPEID::TERRAN_SCV: {
			const Unit* mineral_target = FindNearestMineralPatch(unit->pos); // mineral_target = position of nearest mineral patch
			const Unit* gas_target = FindNearestGasPatch(unit->pos); // gas_target = position of nearest gas patch
			if (!mineral_target) {
                // TryBuildRefinery();
    ...
    //             Actions()->UnitCommand(unit, ABILITY_ID::SMART, gas_target); // send idle worker to gas
		...
		case UNIT_TYPEID::TERRAN_BARRACKS: {
			Actions()->UnitCommand(unit, ABILITY_ID::TRAIN_MARINE); // Train Marine
			break;
		}
		case UNIT_TYPEID::TERRAN_MARINE: {
            ...
            // if we have a marine and we did not attack early, make him a scout
...
void Bot::earlyrush(size_t marinecount, int enemypos) {
	const GameInfo& game_info = Observation()->GetGameInfo();
